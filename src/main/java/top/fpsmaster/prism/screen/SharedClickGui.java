...
public final class SharedClickGui {
    ...
    private void drawChrome(UiFrame ui, ClickGuiBridge bridge, float dt) {
        ...
        if (sideNav(ui, navX, footerY + Metrics.NAV_ITEM + 1f, navW, bridge.i18n("clickgui.nav.music"), "music")) {
            bridge.openMusic();
        }
        if (sideNav(ui, navX, footerY + (Metrics.NAV_ITEM + 1f) * 2f, navW, bridge.i18n("configprofiles.button"), "folder")) {
            bridge.openProfiles();
        }
        if (sideNav(ui, navX, footerY + (Metrics.NAV_ITEM + 1f) * 3f, navW,
    ...
    private float drawMod(UiFrame ui, ClickGuiBridge bridge, ClickGuiBridge.ModInfo mod,
        ...
        if (extra > 0.4f && !mod.settings.isEmpty()) {
            ...
            ui.pushClip(x, y + Metrics.MODULE_ROW, w, extra);
            try {
